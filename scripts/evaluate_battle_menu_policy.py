...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a trained battle-menu throw policy.")
    parser.add_argument("model", help="Path to an SB3 .zip model.")
    ...
    parser.add_argument(
        ...
        choices=["true", "false"],
        default="false",
        help="Use argmax actions. Default false samples from the learned PPO policy distribution.",
    )
    ...
    parser.add_argument(
        "--out",
        default=str(ROOT / "research" / "artifacts" / "battle-menu-policy-evals"),
    )
    ...
    try:
        for state in states:
            for index in range(args.episodes_per_state):
                obs, _ = env.reset(options={"state_path": str(state)})
                terminated = False
                truncated = False
                total_reward = 0.0
                last_info = {}
    ...
    manifest = {
        "schema": "battle_menu_policy_eval_v1",
