...
def navigate_within_pewter_region(
    ...
    *,
    ...
    if landmark is None:
        return SkillResult(
            skill_id=SKILL_ID,
            status="blocked",
            summary=f"Unknown Pewter navigation target: {target!r}.",
            evidence=evidence,
            warnings=warnings,
        )
    ...
    if snapshot.get("battle_type_raw") not in {None, 0} or snapshot.get("mode") == "battle":
        if before_snapshot is not None:
            ...
            return SkillResult(
                skill_id=SKILL_ID,
                status="succeeded",
                summary=f"Pewter navigation reached a {battle_kind} battle waypoint before the requested landmark.",
                evidence=evidence
                + (
        ...
        return SkillResult(
            skill_id=SKILL_ID,
            status="blocked",
            summary="Pewter navigation is interrupted by an active battle.",
            evidence=evidence,
            warnings=warnings,
        )
    ...
    if before_snapshot is not None and snapshot.get("mode") in {"dialogue", "menu_or_dialogue_uncertain"}:
        return SkillResult(
            skill_id=SKILL_ID,
            status="succeeded",
            summary="Pewter navigation reached a forced trainer or story dialogue waypoint before the requested landmark.",
            evidence=evidence
            + (
                f"dialogue_position={position.format() if position else 'unknown'}",
                "dialogue_waypoint=true",
            ),
            warnings=warnings,
        )
    ...
    if snapshot.get("mode") not in {"overworld", "dialogue", "menu_or_dialogue_uncertain"}:
        return SkillResult(
            skill_id=SKILL_ID,
            status="blocked",
            summary="Pewter navigation requires stable overworld or story-dialogue state.",
            evidence=evidence,
            warnings=warnings,
        )

    if before_snapshot is None and snapshot.get("mode") in {"dialogue", "menu_or_dialogue_uncertain"}:
        return SkillResult(
            skill_id=SKILL_ID,
            status="blocked",
            summary="Active dialogue/text must be advanced before starting Pewter navigation.",
            evidence=evidence
            + (
    ...
    if position is None:
        return SkillResult(
            skill_id=SKILL_ID,
            status="uncertain",
            summary="Pewter navigation cannot read the current map position.",
            evidence=evidence,
            warnings=warnings,
        )
    ...
    if not is_allowed_position(position):
        return SkillResult(
            ...
            summary="Current map is outside the Pewter navigation region.",
            evidence=evidence + (f"allowed_maps={_format_allowed_maps()}",),
            warnings=warnings,
        )

    if at_pewter_landmark(position, landmark):
        return SkillResult(
            skill_id=SKILL_ID,
    ...
    return SkillResult(
        skill_id=SKILL_ID,
        status="succeeded",
        summary=f"Pewter navigation can start toward {landmark.label}.",
        evidence=evidence
        + (
...
def _format_allowed_maps() -> str:
    return ",".join(f"0x{map_id:02X}" for map_id in sorted(PEWTER_MAP_IDS))
